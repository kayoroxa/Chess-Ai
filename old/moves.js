...
export function getMoveAndTakePawn({
  type,
  column,
  line,
  side,
  ...
  const { s } = Presets(squareNumbersInUse, column, line)

  return {
    move: [side === 'white' ? s(column, line + 1) : s(column, line - 1)],
    take: [
      side === 'white' ? s(column + 1, line + 1) : s(column - 1, line - 1),
      side === 'white' ? s(column - 1, line + 1) : s(column + 1, line - 1),
    ],
  }
}
